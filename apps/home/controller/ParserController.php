...
class ParserController extends Controller
{
    ...
    public function parserBefore($content)
    {
        ...
        // 实现自动页面关键字及描述
        if (C == 'List') {
            $content = str_replace('{thirteen:pagetitle}', '{sort:name}-{thirteen:sitetitle}-{thirteen:sitesubtitle}', $content);
            $content = str_replace('{thirteen:pagekeywords}', '{sort:keywords}', $content);
            $content = str_replace('{thirteen:pagedescription}', '{sort:description}', $content);
        } elseif (C == 'Content') {
            $content = str_replace('{thirteen:pagetitle}', '{content:title}-{sort:name}-{thirteen:sitetitle}-{thirteen:sitesubtitle}', $content);
            $content = str_replace('{thirteen:pagekeywords}', '{content:keywords}', $content);
            $content = str_replace('{thirteen:pagedescription}', '{content:description}', $content);
        } elseif (C == 'About') {
            $content = str_replace('{thirteen:pagetitle}', '{content:title}-{thirteen:sitetitle}-{thirteen:sitesubtitle}', $content);
            $content = str_replace('{thirteen:pagekeywords}', '{content:keywords}', $content);
            $content = str_replace('{thirteen:pagedescription}', '{content:description}', $content);
        } else {
            $content = str_replace('{thirteen:pagetitle}', '{thirteen:sitetitle}-{thirteen:sitesubtitle}', $content);
            $content = str_replace('{thirteen:pagekeywords}', '{thirteen:sitekeywords}', $content);
            $content = str_replace('{thirteen:pagedescription}', '{thirteen:sitedescription}', $content);
        }
        return $content;
    }
    ...
    {
        $pattern = '/\{thirteen:pre}([\s\S]*?)\{\/thirteen:pre\}/';
    ...
    {
        $content = str_replace('{thirteen:msgaction}', url('/home/Message/add'), $content); // 留言提交路径
        $content = str_replace('{thirteen:checkcode}', CORE_DIR . '/code.php', $content); // 验证码路径
        $content = str_replace('{thirteen:lgpath}', url('/home/Do/area'), $content); // 多语言切换前置路径,如{thirteen:lgpath}?lg=cn
        $content = str_replace('{thirteen:scaction}', url('/home/Search/index'), $content); // 搜索提交路径
        $content = str_replace('{thirteen:appid}', $this->config('api_appid'), $content); // API认证用户
        $content = str_replace('{thirteen:timestamp}', time(), $content); // 认证时间戳
        $content = str_replace('{thirteen:signature}', md5(md5($this->config('api_appid') . $this->config('api_secret') . time())), $content); // API认证密钥
        $content = str_replace('{thirteen:httpurl}', get_http_url(), $content); // 当前访问的域名地址
        $content = str_replace('{thirteen:pageurl}', get_current_url(), $content); // 当前页面的地址
        $content = str_replace('{thirteen:keyword}', get('keyword', 'vars'), $content); // 当前搜索的关键字
        $content = str_replace('{thirteen:checkcodestatus}', $this->config('message_check_code'), $content); // 是否开启验证码
        return $content;
    }
    ...
    {
        $pattern = '/\{thirteen:nav(\s+[^}]+)?\}([\s\S]*?)\{\/thirteen:nav\}/';
        $pattern2 = '/\[nav:([\w]+)(\s+[^]]+)?\]/';
        $pattern3 = '/thirteen:([0-9])+nav/';
        if (preg_match_all($pattern, $content, $matches)) {
            ...
            for ($i = 0; $i < $count; $i ++) {
                ...
                if (preg_match($pattern3, $out_html, $matches3)) {
                    $out_html = str_replace('thirteen:' . $matches3[1] . 'nav', 'thirteen:nav', $out_html);
                ...
                switch ($matches[1][$i]) {
                    ...
                    case 'keywords': // 如果栏目关键字为空，则自动使用全局关键字
                        if ($sort->keywords) {
                            ...
                            $content = str_replace($matches[0][$i], '{thirteen:sitekeywords}', $content);
                        }
                        break;
                    case 'description': // 如果栏目描述为空，则自动使用全局描述
                        if ($sort->description) {
                            ...
                            $content = str_replace($matches[0][$i], '{thirteen:sitedescription}', $content);
                        }
                        break;
                    default:
    ...
    {
        $pattern = '/\{thirteen:sort(\s+[^}]+)?\}([\s\S]*?)\{\/thirteen:sort\}/';
        $pattern2 = '/\[sort:([\w]+)(\s+[^]]+)?\]/';
        if (preg_match_all($pattern, $content, $matches)) {
            $count = count($matches[0]);
            for ($i = 0; $i < $count; $i ++) {
    ...
    {
        $pattern = '/\{thirteen:select(\s+[^}]+)?\}([\s\S]*?)\{\/thirteen:select\}/';
        $pattern2 = '/\[select:([\w]+)(\s+[^]]+)?\]/';
        if (preg_match_all($pattern, $content, $matches)) {
            $count = count($matches[0]);
    ...
    {
        $pattern = '/\{thirteen:list(\s+[^}]+)?\}([\s\S]*?)\{\/thirteen:list\}/';
        $pattern2 = '/\[list:([\w]+)(\s+[^]]+)?\]/';
        if (preg_match_all($pattern, $content, $matches)) {
            $count = count($matches[0]);
            for ($i = 0; $i < $count; $i ++) {
    ...
    {
        $pattern = '/\{thirteen:content(\s+[^}]+)?\}([\s\S]*?)\{\/thirteen:content\}/';
    ...
    {
        $pattern = '/\{thirteen:pics(\s+[^}]+)?\}([\s\S]*?)\{\/thirteen:pics\}/';
        $pattern2 = '/\[pics:([\w]+)(\s+[^]]+)?\]/';
        if (preg_match_all($pattern, $content, $matches)) {
            $count = count($matches[0]);
            for ($i = 0; $i < $count; $i ++) {
    ...
    {
        $pattern = '/\{thirteen:checkbox(\s+[^}]+)?\}([\s\S]*?)\{\/thirteen:checkbox\}/';
    ...
    {
        $pattern = '/\{thirteen:tags(\s+[^}]+)?\}([\s\S]*?)\{\/thirteen:tags\}/';
        $pattern2 = '/\[tags:([\w]+)(\s+[^]]+)?\]/';
        if (preg_match_all($pattern, $content, $matches)) {
            $count = count($matches[0]);
            for ($i = 0; $i < $count; $i ++) {
    ...
    {
        $pattern = '/\{thirteen:slide(\s+[^}]+)?\}([\s\S]*?)\{\/thirteen:slide\}/';
        $pattern2 = '/\[slide:([\w]+)(\s+[^]]+)?\]/';
        if (preg_match_all($pattern, $content, $matches)) {
            $count = count($matches[0]);
            for ($i = 0; $i < $count; $i ++) {
    ...
    {
        $pattern = '/\{thirteen:link(\s+[^}]+)?\}([\s\S]*?)\{\/thirteen:link\}/';
        $pattern2 = '/\[link:([\w]+)(\s+[^]]+)?\]/';
        if (preg_match_all($pattern, $content, $matches)) {
            $count = count($matches[0]);
            for ($i = 0; $i < $count; $i ++) {
    ...
    {
        $pattern = '/\{thirteen:message(\s+[^}]+)?\}([\s\S]*?)\{\/thirteen:message\}/';
    ...
    {
        $pattern = '/\{thirteen:formlist(\s+[^}]+)?\}([\s\S]*?)\{\/thirteen:formlist\}/';
        $pattern2 = '/\[form:([\w]+)(\s+[^]]+)?\]/';
        if (preg_match_all($pattern, $content, $matches)) {
            $count = count($matches[0]);
            for ($i = 0; $i < $count; $i ++) {
    ...
    {
        $pattern = '/\{thirteen:search(\s+[^}]+)?\}([\s\S]*?)\{\/thirteen:search\}/';
        $pattern2 = '/\[search:([\w]+)(\s+[^]]+)?\]/';
        if (preg_match_all($pattern, $content, $matches)) {
            $count = count($matches[0]);
    ...
    {
        $pattern = '/\{thirteen:loop(\s+[^}]+)?\}([\s\S]*?)\{\/thirteen:loop\}/';
        if (preg_match_all($pattern, $content, $matches)) {
            $count = count($matches[0]);
            for ($i = 0; $i < $count; $i ++) {
                // 获取调节参数
    ...
    {
        $pattern = '/\{thirteen:if\(([^}]+)\)\}([\s\S]*?)\{\/thirteen:if\}/';
        $pattern2 = '/thirteen:([0-9])+if/';
        if (preg_match_all($pattern, $content, $matches)) {
            $count = count($matches[0]);
            for ($i = 0; $i < $count; $i ++) {
                $flag = '';
                ...
                if (preg_match($pattern2, $out_html, $matches3)) {
                    $out_html = str_replace('thirteen:' . $matches3[1] . 'if', 'thirteen:if', $out_html);
        ...
        switch ($label) {
            ...
            case 'keywords': // 如果内容关键字为空，则自动使用全局关键字
                if ($data->keywords) {
                    ...
                    $content = str_replace($search, '{thirteen:sitekeywords}', $content);
                }
                break;
            case 'description': // 如果内容描述为空，则自动使用全局描述
                if ($data->description) {
                    ...
                    $content = str_replace($search, '{thirteen:sitedescription}', $content);
                }
                break;
            default:
                if (isset($data->$label)) {
